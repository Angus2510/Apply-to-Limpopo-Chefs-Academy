...
const steps = [
  ...
  },
  {
    title: 'Motivation',
    route: 'step-five',
    link: ApplyRoutes.STUDENT_UPLOAD,
  },
    ...
    <Card className="w-full flex flex-col justify-between">
      ...
      <CardContent className="flex flex-col gap-4">
        <div className="relative flex flex-row justify-between lg:flex-col lg:justify-start lg:gap-8 pt-4">
          {steps.map((step, i) => (
            <Link href={step.link} key={step.link} className="group z-20 flex items-center gap-3 text-lg"> {/* Changed text-2xl to text-lg */}
              <span className={cn(
              ...
              <span className={cn(
                ...
                pathname.includes(step.route) ? 'font-semibold text-green-700' : 'font-light text-gray-500 group-hover:text-gray-700'
              )}>
                {step.title}
              </span>
            </Link>
