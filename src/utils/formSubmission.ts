...
export const handleFinalSubmission = async (data: NewApplyInitialValuesType): Promise<void> => {
  try {
    ...
    const videoUrl = data.videoUpload || ''; // Provide a default empty string if videoUpload is undefined
    await sendEmail(pdfBuffer, videoUrl, data);
...
const generatePDF = async (data: NewApplyInitialValuesType): Promise<Buffer> => {
  const doc = new jsPDF();
  const margin = 10;

  // Helper function to draw a section with autoTable
  const drawSection = (title: string, content: Record<string, string | boolean>, startY: number, doc: jsPDF) => {
    ...
    autoTable(doc, {
      ...
      didDrawPage: (dataArg) => {
        if (dataArg.cursor && dataArg.cursor.y > doc.internal.pageSize.height - 20) {
          doc.addPage();
        }
      },
    });
  ...
  // Add header
  doc.setFontSize(20).setTextColor(40, 40, 40).text('Application Form', 105, 30, { align: 'center' });
  ...
  yPos = drawSection('Guardian Details', {
  ...
  yPos = drawSection('Study Details', {
  ...
  yPos = drawSection('Course and Campus', {
  ...
  yPos = drawSection('Uploads', {
    'Video Upload': data.videoUpload || '',
  }, yPos + 10, doc);
...
const sendEmail = async (pdfBuffer: Buffer, videoUrl: string, data: NewApplyInitialValuesType) => {
  ...
  // const campusEmails = {
  //   Mokopane: 'victor@swartdigital.co.za',
  //   Polokwane: 'victor@swartdigital.co.za',
  // };
  ...
  const mailOptionsCampus = {
    ...
    text: `Application form submitted by ${data.studentName} ${data.studentSurname}. Attached is the application PDF and the video URL is ${videoUrl}`,
    attachments: [
      {
  ...
  //   text: `Dear ${data.studentName},\n\nThank you for your application. We have received it and will process it shortly.\n\nBest regards.`,
  // };
   ...
   try {
    ...
    // console.log('Student email sent successfully:', infoStudent);
  } catch (error) {
